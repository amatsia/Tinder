...
class ScrollViewController: UIViewController, UIScrollViewDelegate, UIGestureRecognizerDelegate, UIPageViewControllerDelegate, UIPopoverPresentationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        facebookLoginButton.titleLabel?.font = UIFont.systemFont(ofSize: 19)
        ...
        // Phone Button
        let phoneLoginButton = UIButton(frame: CGRect(x: 60, y: (screenSize.height*0.6)+85, width: screenSize.width-120, height: 40))
        ...
        phoneLoginButton.titleLabel?.font = UIFont.systemFont(ofSize: 17)
    ...
    func getFacebookUserInfo() {
        if(FBSDKAccessToken.current() != nil)
        {
            ...
            let graphRequest = FBSDKGraphRequest(graphPath: "me", parameters: ["fields" : "id, name, first_name, picture.type(large), email"])
            let connection = FBSDKGraphRequestConnection()
            connection.add(graphRequest, completionHandler: { (connection, result, error) -> Void in
                let data = result as! [String : AnyObject]
                userName = (data["first_name"] as? String)!
                let FBid = data["id"] as? String
                let url = NSURL(string: "https://graph.facebook.com/\(FBid!)/picture?type=large&return_ssl_resources=1")
                userProfileImageView.image = UIImage(data: NSData(contentsOf: url! as URL)! as Data)
            })
            connection.start()
        }
    }
